...
{
	VideoData video = g_pState->Gallery.GetVideo(videoIndex);
	if (video.FullPath.empty()) return;

	std::filesystem::path ffmpegPath = g_pState->FFmpeg.GetExecutablePath();
	...
	if (!imageData.empty()) 
	{
		texturePtr = g_pSystem->Render.CreateTextureFromMemory(imageData.data(), imageData.size());
	}

	g_pState->Gallery.UpdateVideoMetadata(videoIndex, duration, texturePtr, video.FullPath.string());
}
...
void GallerySystem::RefreshList(const std::string& path)
{
	...
	g_pState->Gallery.SetScanning(true);

	std::vector<VideoData> newBatch;
	const auto& oldVideos = g_pState->Gallery.GetVideos();
	...
	g_pState->Gallery.SetVideos(std::move(newBatch));
	g_pState->Gallery.SetScanning(false);
}
...
void GallerySystem::DeleteVideo(int videoIndex)
{
	...
	g_pState->Gallery.DeleteVideo(videoIndex);
}
...
void GallerySystem::RenameVideo(int videoIndex, const std::string& newName)
{
	...
	try 
	{
		...
		g_pState->Gallery.UpdateVideoName(videoIndex, finalName);
