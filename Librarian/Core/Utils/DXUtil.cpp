...
DX11Addresses DXUtil::GetVtableAddresses() 
{
    ...
    {
        g_pUtil->Log.Append("[DXUtil] ERROR: RegisterClassEx failed");
        return addr;
    }
    ...
    if (!hWnd) 
    {
        g_pUtil->Log.Append("[DXUtil] ERROR: CreateWindowEx failed");
    ...
    {
        g_pUtil->Log.Append("[DXUtil] ERROR: D3D11CreateDeviceAndSwapChain failed");
    }
